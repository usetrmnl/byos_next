...
function generateSvgData(
  ...
  Array.from(text).forEach((char) => {
    ...
    const binaryArray = binaryString.padEnd(fontWidth * fontHeight, '0').slice(0, fontWidth * fontHeight);
    const pathData = Array.from({ length: fontWidth * fontHeight }).map((_, i) => {
  ...
  return (
    ...
    >
      {svgData.paths.map((item: { path: string; x: number }, index: number) => (
        <g 
          key={index} 
          transform={`translate(${item.x}, 0) scale(${scale})`}
        >
          <path 
            d={item.path} 
            fill="currentColor" 

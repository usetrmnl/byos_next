...
export const precacheImageInBackground = (
    ...
    fetch(imageUrl, { method: "GET" })
        .then((response) => {
            ...
            logInfo("Image pre-cached successfully", { source: "api/display", metadata: { imageUrl, friendlyId } });
        })
        .catch((error: Error) => {
            logError("Failed to precache image", { source: "api/display", metadata: { imageUrl, error: error.message, friendlyId } });
        });
};
...
export const getActivePlaylistItem = async (
    ...
    if (!items || items.length === 0) {
        logError("No items in playlist", { source: "api/display", metadata: { playlistId } });
        return null;
    }
    ...
    const metadata = { playlistId, currentIndex, timezone, currentTime, currentDay, totalItems: items.length };
    logInfo("Checking playlist items for time/day match", { source: "api/display", metadata });
...
export const updateDeviceStatus = async (
    ...
    try {
        ...
        logError("Error updating device status", { source: "api/display", metadata: { deviceId: device.id, headers } });
    }
};
        ...
        if (deviceByApiKey) {
            ...
            if (macAddress && macAddress !== device.mac_address) {
                await db.updateTable("devices")
                    .set({ mac_address: macAddress, updated_at: new Date().toISOString() })
                    .where("id", "=", device.id.toString())
                    .execute();
                logInfo("Updated MAC address for device", { source: "api/display", metadata: { deviceId: device.friendly_id } });
            }
            return device;
        }
    }
        ...
        if (deviceByMac) {
            ...
            if (apiKey && apiKey !== device.api_key) {
                await db.updateTable("devices")
                    ...
                    .where("id", "=", device.id.toString())
                    .execute();
                logInfo("Updated API key for device", { source: "api/display", metadata: { deviceId: device.friendly_id } });
            }
            return device;
        }
    }
    ...
    if (apiKey) {
        ...
        if (macAddress) {
            ...
            try {
                const newDevice = await db
                    ...
                    .values({
                        ...
                        refresh_schedule: JSON.stringify({
                            default_refresh_rate: headers.refreshRate ? Number.parseInt(headers.refreshRate, 10) : 60,
                            time_ranges: [],
                        }),
                ...
                if (newDevice) {
                    logInfo("Created new device with provided MAC address", { source: "api/display", metadata: { friendly_id } });
                    return newDevice as unknown as Device;
                }
            } catch (e) {
                logError("Error creating device with provided MAC", { source: "api/display", metadata: { error: e } });
            }
        }

        // Mock Device logic
        const mockMacAddress = generateMockMacAddress(apiKey);
        const existingMock = await db.selectFrom("devices").selectAll().where("mac_address", "=", mockMacAddress).executeTakeFirst();

        if (existingMock) {
            const device = existingMock as unknown as Device;
            if (macAddress) {
                await db.updateTable("devices").set({ mac_address: macAddress }).where("id", "=", device.id.toString()).execute();
            }
            logger.info("Using existing mock device", { friendly_id: device.friendly_id });
            return device;
        }

        // Create Mock Device
        const friendly_id = generateFriendlyId(mockMacAddress, new Date().toISOString().replace(/[-:Z]/g, ""));
        const new_api_key = macAddress ? apiKey : generateApiKey(mockMacAddress, new Date().toISOString().replace(/[-:Z]/g, ""));

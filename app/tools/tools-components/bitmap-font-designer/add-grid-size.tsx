...
export default function AddGridSize({ 
        ...
        requestAnimationFrame(() => {
            ...
            for (let rowIdx = 1; rowIdx <= maxGridSize; rowIdx++) {
                for (let colIdx = 1; colIdx <= maxGridSize; colIdx++) {
                    ...
                    const isDisabled = availableGridSizes.includes(size) || 
                                    (colIdx <= 4 && rowIdx <= 4)
    ...
    const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        const isDisabled = availableGridSizes.includes(hoveredSize) || 
                          (colIdx <= 4 && rowIdx <= 4)
    ...
    return (
        <DropdownMenu open={open} onOpenChange={setOpen}>
            <DropdownMenuTrigger asChild><Button variant="outline" size="sm">Add Grid Size</Button></DropdownMenuTrigger>
            <DropdownMenuContent className="p-2" onCloseAutoFocus={(e) => e.preventDefault()}>

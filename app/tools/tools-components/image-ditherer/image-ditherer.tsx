...
import { Upload, Download, ImageIcon, ZoomIn, ZoomOut, Sliders, Copy } from "lucide-react"
...
export default function ImageDitherer() {
  ...
  const applyThreshold = (imageData: ImageData, threshold: number, inverted: boolean) => {
    const data = imageData.data
    for (let i = 0; i < data.length; i += 4) {
      const gray = data[i]
  ...
  const applyFloydSteinberg = (imageData: ImageData, inverted: boolean) => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (y + 1 < height && x > 0) grayscale[index + width - 1] += (error * 3) / 16
        if (y + 1 < height) grayscale[index + width] += (error * 5) / 16
        if (y + 1 < height && x + 1 < width) grayscale[index + width + 1] += (error * 1) / 16
      }
    }
  ...
  const applyAtkinson = (imageData: ImageData, inverted: boolean) => {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (y + 1 < height && x + 1 < width) grayscale[index + width + 1] += error
  ...
  const applyBayer = (imageData: ImageData, patternSize: number, inverted: boolean) => {
    const width = imageData.width
    const height = imageData.height
    const data = imageData.data
  ...
  }, [originalImage, ditheringMethod, brightness, contrast, threshold, patternSize, inverted, applyDithering])
            ...
            <div className="space-y-4">
              ...
              {ditheringMethod === "threshold" && (
                <div className="space-y-1">
                  <label className="text-xs font-medium">Threshold ({threshold[0]})</label>
                  <Slider value={threshold} min={0} max={255} step={1} onValueChange={setThreshold} className="h-4" />
                </div>
              )}
              ...
              {ditheringMethod === "bayer" && (
                <div className="space-y-1">
                  ...
                  <Slider value={patternSize} min={2} max={8} step={2} onValueChange={setPatternSize} className="h-4" />
                </div>
              )}

              <div className="space-y-1">
                <label className="text-xs font-medium">Brightness ({brightness[0]})</label>
                <Slider
                  value={brightness}
                  min={-200}
                  max={200}
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium">Contrast ({contrast[0]})</label>
                <Slider value={contrast} min={-100} max={100} step={1} onValueChange={setContrast} className="h-4" />
              </div>

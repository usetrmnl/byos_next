 ...
 */
async function geocodeLocation(locationName: string): Promise<{ latitude: number; longitude: number; name: string } | null> {
    try {
 ...
 */
async function getWeatherData(latitude?: number, longitude?: number, locationName?: string): Promise<WeatherData | null> {
    try {
 ...
 */
async function fetchWeatherDataNoCache(latitude?: number, longitude?: number, locationName?: string): Promise<WeatherData> {
...
const getCachedWeatherData = unstable_cache(
    async (latitude?: number, longitude?: number, locationName?: string): Promise<WeatherData> => {
        const data = await getWeatherData(latitude, longitude, locationName);
...
export default async function getData(props?: any): Promise<WeatherData> {
    ...
    try {
        ...
        return await getCachedWeatherData(finalLatitude, finalLongitude, finalLocationName);
    } catch (error) {
        console.log("Cache skipped or error:", error);
        // Fall back to non-cached data
        console.log("Fallback to non-cached data: ", finalLatitude, finalLongitude, finalLocationName);
        return fetchWeatherDataNoCache(finalLatitude, finalLongitude, finalLocationName);
    }
} 
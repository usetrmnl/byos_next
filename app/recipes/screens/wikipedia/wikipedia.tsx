...
export default function Wikipedia({
	...
	const safeTitle = title || displaytitle?.replace(/<[^>]*>?/g, "").trim() || "Wikipedia Article"; // display title contains html, need to be stripped
	...
	const safeContentUrl = fullurl || content_urls?.desktop?.page || "https://en.wikipedia.org";
	...
	const getImageDimensions = () => {
		if (!hasValidThumbnail || !thumbnail) return { width: "240px", height: "auto" };
		...
		try {
			...
			const height = typeof thumbnail.height === 'number' ? thumbnail.height : 180;
					...
					{transform(
						<div className="flex flex-col w-[800px] h-[480px]">
							...
							<div className="flex-1 p-4 flex flex-row">
								<div className="text-2xl flex-grow tracking-tight leading-none" style={{ textOverflow: "ellipsis", maxHeight: "240px" }}
								>
									{truncatedExtract}
								</div>
							...
							<div className="flex-none p-4 flex flex-col">
								...
								>
									...
									<span>{safeDescription && safeDescription.length > 100 ? safeDescription.substring(0, 100) + "..." : safeDescription}</span>

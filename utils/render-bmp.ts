...
const applyDithering = (
  ...
  switch (method) {
    ...
    case DitheringMethod.THRESHOLD:
      return ditheringThreshold(grayscale, 128, grayscaleLevels);
    default:
      return ditheringFloydSteinberg(grayscale, width, height, grayscaleLevels);
  }
};
...
/**
 * Convert a PNG buffer to a simple BMP with the specified bit depth and dithering
 */
export async function toBitDepth(
  png: Buffer,
  options: ToBitDepthOptions = {},
): Promise<Buffer> {
  const { grayscale = 2, ditheringMethod = DitheringMethod.FLOYD_STEINBERG } =
    options;
  ...
  // Apply dithering if a method is specified
  const grayscaleLevels = grayscale;
    ...
    if (bitsPerPixel === 1) {
      for (let x = 0; x < targetWidth; x += 8) {
        ...
        for (let bit = 0; bit < remaining; bit++) {
          ...
          const paletteIndex = valueToIndex(ditheredData[idx]);
          if (paletteIndex === grayscale - 1) {
            byte |= 1 << (7 - bit);
          }
        }

...
const glyphChar = (glyph, key) =>
	glyph.char?.length ? glyph.char : String.fromCharCode(glyph.charCode ?? Number(key));

export function shiftGlyphsToMetrics(glyphs, metrics) {
...
export function deriveTypographyMetrics(glyphs, fallback) {
	...
	for (const [key, glyph] of Object.entries(glyphs)) {
		...
		if (CAP_CHARS.test(char)) capMaxY = Math.max(capMaxY, boundsMaxY);
		if (DESC_CHARS.test(char)) descMinY = Math.min(descMinY, boundsMinY);
		if (X_HEIGHT_CHARS.test(char)) xHeightMaxY = Math.max(xHeightMaxY, boundsMaxY);
	}
	...
	const accentPad = 1;
	const maxY = Number.isFinite(capMaxY) ? capMaxY + accentPad : inkMaxY + accentPad;
...
export function applyDerivedMetricsToV2Pack(pack, options = {}) {
...
export function applyManualMetricsFromSource(v2Pack, source) {
	...
	for (const grid of source.bitmapGrids) {
		...
		if (!height) continue;
		const faceKey =
			grid.width > 0 ? `${grid.width}x${height}` : `0x${height}`;
...
export function manualFaceKeysFromSource(source) {
